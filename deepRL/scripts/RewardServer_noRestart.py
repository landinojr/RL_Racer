...
def computeReward(msg):
    ...
    print(str(msg.checkpoint))
    if (msg.new_episode):
        checkpointStart = rospy.Time.now().to_sec()
    if (msg.checkpoint == len(xs)):
        raise Exception('msg.checkpoint is too high')
        cpx = xs[0]
        cpy = ys[0]
    else:
    ...
    if (d < curDistance):
        ...
        response = reward_requestResponse()
        if (isCollision):
            response.reward = timeCost - progressReward - 1.0
        else:
            response.reward = timeCost - progressReward
